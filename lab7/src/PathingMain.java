...
public class PathingMain extends PApplet
{
   ...
   public void keyPressed()
   {
      if (key == ' ')
      {
         ...
         path.clear();
            ...
            dfs(new Point(wPos.x + 1, wPos.y), grid, path);
   ...
   private boolean dfs(Point pos, GridValues[][] grid, List<Point> path)
   {
...

      {
         ...
         if(nextMove(rightN.x, rightN.y + 1, grid)) {
            dfs(new Point(pos.x, pos.y + 1), grid, path);
         }
         else if(nextMove(rightN.x  - 1, rightN.y, grid)) {
              dfs(new Point(pos.x - 1, pos.y), grid, path);
         }
         else if(nextMove(rightN.x  + 1, rightN.y, grid)) {
            dfs(new Point(pos.x + 1, pos.y), grid, path);
         }
         else if(nextMove(rightN.x, rightN.y - 1, grid)) {
            dfs(new Point(pos.x, pos.y - 1), grid, path);
         }
   ...
   public static boolean nextMove(int x, int y, GridValues[][] grid) {

      if(!withinBounds(new Point(x, y), grid)) {
         return false;
      }
      if(grid[y][x] == GridValues.SEARCHED) {
         return false;
      }
      if(grid[y][x] == GridValues.OBSTACLE) {
         return false;
      }
      return true;
   }
